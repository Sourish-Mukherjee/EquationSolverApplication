...
def letter_seg(lines_img, x_lines, i):
	...
	for e in range(len(letter)):
		if(letter[e][0]<x_linescopy[0]):
			...
			cv2.imwrite(os.path.join(OUTPUT_DIR, str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.png'), 255-letter_img)
		else:
			x_linescopy.pop(0)
			word += 1
			letter_index = 1
			...
			cv2.imwrite(OUTPUT_DIR+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.png', 255-letter_img)
